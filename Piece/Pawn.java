...
public class Pawn extends Piece {
    ...
    public void setPossibleMoves() {
        ...
        if (alignment == PieceAlignment.BLACK) {
            if (y + 1 < 8) {
                // Se a casa seguinte nao estiver ocupada ou e estiver dentro do tabuleiro,
                // adiciona a lista de movimentos possiveis
                if (!board.getTile(x, y + 1).getIsTileOccupied()) {
                    moves.add(board.getTile(x, y + 1));
                }
                // Se for o primeiro movimento do peao e a casa com y+2 nao estiver ocupada,
                // adiciona a lista de movimentos possiveis
                if (firstMove) {
                    if (y + 2 < 8) {
            ...
            // Se as casas das diagonais adjacentes estiverem ocupadas por pecas de outra
            // cor, adiciona elas a lista de movimentos possiveis
            if (x + 1 < 8 && y + 1 < 8) {
